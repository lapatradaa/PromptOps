...
class ShotTemplateFormatter:
    ...
    def format_all_rows(self, shot_type='zero', perturb_type=None):
        ...
        for _, row in self.df.iterrows():
            if shot_type == 'zero':
                original, perturbed = self.format_zero_shot(row, perturb_type=perturb_type)
            elif shot_type == 'one':
                original, perturbed = self.format_one_shot(row, perturb_type=perturb_type)
            elif shot_type == 'few':
                original, perturbed = self.format_few_shot(row, perturb_type=perturb_type)
            else:
